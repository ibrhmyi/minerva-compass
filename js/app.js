...
(function () {
  ...
  var isEmbedMode = new URLSearchParams(window.location.search).get('embed') === '1';
  var userStage = null; // pre-chat survey stage
  var requestStartTime = 0; // for response time tracking
  var idleTimer = null; // proactive nudge timer
  var nudgeShown = false;
  var sessionFlagged = false; // user-flagged session
  ...
  function init() {
    ...
    // Add history toggle button to header (skip in embed mode)
    if (!isEmbedMode) addHistoryButton();
    ...
    try { emailCaptured = !!localStorage.getItem('minerva-compass-email-captured'); } catch (e) {}

    // Load saved user stage
    try { userStage = localStorage.getItem('minerva-compass-stage'); } catch (e) {}

    // Show pre-chat survey if no stage set and no conversation active
    if (!userStage && conversationHistory.length === 0) {
      showPreChatSurvey();
    }
  ...
  function createStreamingMessage() {
    ...
    // Show typing indicator until first text arrives
    contentDiv.innerHTML = '<div class="thinking-indicator"><span></span><span></span><span></span><span class="thinking-label">Compass is thinking...</span></div>';

    contentWrapper.appendChild(contentDiv);
    msgDiv.appendChild(contentWrapper);
    chatArea.appendChild(msgDiv);

    return { msgDiv: msgDiv, contentDiv: contentDiv, contentWrapper: contentWrapper, firstChunk: true };
  }
  ...
  function updateStreamContent(contentDiv, text) {
    ...
    // Strip follow-ups during streaming (both |||FOLLOWUPS||| and bare |||)
    var markerIdx = text.indexOf('|||FOLLOWUPS|||');
    if (markerIdx !== -1) {
      displayText = text.substring(0, markerIdx);
    } else {
      var pipeIdx = text.indexOf('|||');
      if (pipeIdx !== -1) displayText = text.substring(0, pipeIdx);
    }
  ...
  function finalizeStreamMessage(streamMsg, text, originalQuestion) {
    ...
    // Track assistant messages for email capture
    ...
    flagBtn.className = 'feedback-btn flag-btn';
    flagBtn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="13" height="13"><path d="M4 15s1-1 4-1 5 2 8 2 4-1 4-1V3s-1 1-4 1-5-2-8-2-4 1-4 1z"/><line x1="4" y1="22" x2="4" y2="15"/></svg>';
    flagBtn.title = 'Flag this response';
    flagBtn.addEventListener('click', function () {
      if (flagBtn.classList.contains('active')) return;
      flagBtn.classList.add('active', 'flagged');
      sessionFlagged = true;
      // Mark the assistant message in history as flagged
      // The message will be pushed after finalizeStreamMessage returns,
      // so we use a short delay to ensure it's in the array
      setTimeout(function () {
        // Find the last assistant message near this index
        for (var fi = conversationHistory.length - 1; fi >= 0; fi--) {
          if (conversationHistory[fi].role === 'assistant' && conversationHistory[fi].content === cleanText) {
            conversationHistory[fi].flagged = true;
            break;
          }
        }
        syncSession();
    ...
    // Response time — inline in actions row
    if (requestStartTime) {
    ...
    // Show email capture after 3rd assistant message (once)
    if (assistantMessageCount === 3 && !emailCaptured && !isEmbedMode) {
      setTimeout(function () { showEmailCapture(); }, 800);
    }
  ...
  function parseFollowUps(text) {
    // Method 1: Look for |||FOLLOWUPS||| marker
    ...
    // Method 2: AI sometimes skips FOLLOWUPS marker and just outputs |||q1|||q2|||q3
    var pipeIdx = text.indexOf('|||');
    if (pipeIdx !== -1) {
      var cleanText = text.substring(0, pipeIdx).trim();
      var remainder = text.substring(pipeIdx);
      var parts = remainder.split('|||').map(function (s) { return s.trim(); }).filter(Boolean);
      var followUps = parts.filter(function (q) { return q.length > 5 && q.length < 80; }).slice(0, 3);
      if (followUps.length > 0) {
        return { cleanText: cleanText, followUps: followUps };
      }
  ...
  function showEmailCapture() {
    ...
    if (document.querySelector('.email-capture')) return;

    var captureDiv = document.createElement('div');
    captureDiv.className = 'email-capture glass';
    captureDiv.innerHTML =
      '<div class="email-capture-text">' +
        '<strong>Want admissions updates?</strong>' +
        '<span>Leave your email and we\'ll keep you posted on deadlines and events.</span>' +
      '</div>' +
      '<div class="email-capture-form">' +
        '<input type="email" class="email-capture-input" placeholder="your@email.com" autocomplete="email">' +
        '<button class="email-capture-btn" type="button">Send</button>' +
      '</div>' +
      '<button class="email-capture-dismiss" type="button" aria-label="Dismiss">&times;</button>';

    chatArea.appendChild(captureDiv);
    scrollToBottom();

    var input = captureDiv.querySelector('.email-capture-input');
    var btn = captureDiv.querySelector('.email-capture-btn');
    var dismiss = captureDiv.querySelector('.email-capture-dismiss');
    ...
    btn.addEventListener('click', function () {
      ...
      // Get conversation context (first user message)
      var context = '';
      for (var i = 0; i < conversationHistory.length; i++) {
        if (conversationHistory[i].role === 'user') {
          context = conversationHistory[i].content.slice(0, 200);
          break;
        }
      }
      ...
      fetch('/api/emails', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: email, context: context }),
      }).catch(function () {});

      emailCaptured = true;
      try { localStorage.setItem('minerva-compass-email-captured', '1'); } catch (e) {}

      // Hide form elements instead of replacing innerHTML (Safari compat)
      captureDiv.querySelector('.email-capture-text').style.display = 'none';
      captureDiv.querySelector('.email-capture-form').style.display = 'none';
      dismiss.style.display = 'none';
      var successDiv = document.createElement('div');
      successDiv.className = 'email-capture-success';
      successDiv.textContent = "Thanks! We'll keep you posted.";
      captureDiv.appendChild(successDiv);

      setTimeout(function () {
        captureDiv.style.animation = 'fadeOut 0.3s ease forwards';
        setTimeout(function () { captureDiv.remove(); }, 300);
      }, 2000);
    });
    ...
    try {
      fetch('/api/sessions', {
        ...
        body: JSON.stringify({
          ...
          userFlagged: sessionFlagged || undefined,
        }),
      }).catch(function () {});
    } catch (e) {}
  }
  ...
  // --- Pre-Chat Survey ---
  function showPreChatSurvey() {
    if (!welcomeScreen) return;

    var survey = document.createElement('div');
    survey.className = 'pre-chat-survey';
    survey.innerHTML =
      '<p class="survey-question">Where are you in your journey?</p>' +
      '<div class="survey-options">' +
        '<button class="survey-btn glass-subtle" data-stage="exploring">Just exploring</button>' +
        '<button class="survey-btn glass-subtle" data-stage="applying">Applying soon</button>' +
        '<button class="survey-btn glass-subtle" data-stage="admitted">Admitted</button>' +
        '<button class="survey-btn glass-subtle" data-stage="parent">Parent / Guardian</button>' +
      '</div>';

    var heroEl = welcomeScreen.querySelector('.welcome-hero');
    if (heroEl && heroEl.nextSibling) {
      welcomeScreen.insertBefore(survey, heroEl.nextSibling);
    } else {
      welcomeScreen.appendChild(survey);
    }

    var buttons = survey.querySelectorAll('.survey-btn');
    buttons.forEach(function (btn) {
      btn.addEventListener('click', function () {
        userStage = btn.dataset.stage;
        try { localStorage.setItem('minerva-compass-stage', userStage); } catch (e) {}

        // Track stage selection immediately (fire-and-forget)
        try {
          fetch('/api/feedback', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ type: 'stage', stage: userStage }),
          }).catch(function () {});
        } catch (e) {}
        ...
        setTimeout(function () {
          survey.style.animation = 'fadeOut 0.3s ease forwards';
          setTimeout(function () { survey.remove(); }, 300);
        }, 500);
      });
    });
  }
  ...
  function startIdleTimer() {
    resetIdleTimer();
    // Listen for any interaction to reset
  ...
  function resetIdleTimer() {
    ...
    if (nudgeShown || conversationHistory.length > 0 || isEmbedMode) return;
    idleTimer = setTimeout(showIdleNudge, 30000); // 30 seconds
  }

  function showIdleNudge() {
    if (nudgeShown || conversationHistory.length > 0) return;
    nudgeShown = true;

    var nudge = document.createElement('div');
    nudge.className = 'idle-nudge';
    nudge.innerHTML = 'Not sure where to start? Try asking about <button class="nudge-link">deadlines</button> or <button class="nudge-link">how to apply</button>';

    var inputArea = document.querySelector('.input-area');
    if (inputArea) {
      inputArea.insertBefore(nudge, inputArea.firstChild);
    }

    nudge.querySelectorAll('.nudge-link').forEach(function (link) {
      link.addEventListener('click', function () {
        var text = link.textContent === 'deadlines' ? 'What are the application deadlines?' : 'How do I apply to Minerva?';
    ...
    setTimeout(function () {
      if (nudge.parentNode) {
        nudge.style.animation = 'fadeOut 0.3s ease forwards';
